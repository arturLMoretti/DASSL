...
C
      ...
      EXTERNAL RESWEB, JACRS, PSOLRS, QRES
C
...
C 
 ...
 50   INFO(I) = 0
c      info(9) = 1  
c     iwork(3) = 2       ! change maximum order
C
...
C is to be done with two parameters
      ...
      print *, ' method for computing sensitivity residues:(0,1)'
...
C
      ...
      if (info(28) .gt. 0) then
         neq = (ny + info(28))*(1+info(19))
         do i = 1, 1+info(19)
            do j = 1, info(28)
               cc((ny+j)+(i-1)*(ny+info(28))) = 0
               ccprime((ny+j)+(i-1)*(ny+info(28))) = 0.0d0
            end do
         end do
      end if
      info(18) = 1
...
c           write(10, *) (cc(j), j=ny+1, 2*ny)
          ...
          NNI = 0
       ENDIF
c       print *, (cc(ny+1)*j, j = 1, info(19)+1)
 200    CONTINUE
        CALL DSENSD(
     *       QRES, NEQ, T, CC, CCPRIME, QSEN, INFO, 
     *       RWORK, IWORK, IRES, RPAR, IPAR, SENPAR)
c
        print *, '\n \\int\\sum y_i and its sensitivities:'
        print *, (cc(ny+1)*j, j = 1, info(19)+1)
        print *, '\n \\sum y_i**2 and its sensitivities:'
        print *, (qsen(i),i=1,3)
C
 210  CONTINUE
      LENRW = IWORK(18)
...
C
 ...
 300   CONTINUE
c      CALL OUTWEB (T, CC, NS, MX, MY, LCOUT)
c      CALL OUTWEB (T, CC(ny+1), NS, MX, MY, LCOUT+1)
c      CALL OUTWEB (T, CC(2*ny+1), NS, MX, MY, LCOUT+2)
...
C
c      WRITE(LUN,10) T
c 10   FORMAT(/1X,79('-')/30X,'At time t = ',E16.8/1X,79('-') )
C
c      DO 40 I = 1,NS
c        WRITE(LUN,20) I
c 20     FORMAT(' the species c(',i2,') values are ')
        DO 30 JY = MY,1,-1
           do jx = 1, mx
              write(lun, *) jx, jy, (c(i,jx, jy), i=1,ns)
           end do
           write(lun,*)
c          WRITE(LUN,25) (C(I,JX,JY),JX=1,MX)
c 25       FORMAT(6(1X,G12.6))
 30       CONTINUE
          write(lun, *)
c        WRITE(LUN,35)
c 35     FORMAT(1X,79('-'),/)
