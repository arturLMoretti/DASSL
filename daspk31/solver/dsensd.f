...
C***REVISION DATE  000719   (Bug fixed in dradfmv)
C***REVISION DATE  070116   (Bug fixed to return Q in QSEN for central difference meth)
C
...
C
C                 SUBROUTINE QRES(T,Y,G_Y,YPRIME,G_YPRIME,DELTA,G_DELTA,IRES,
...
C
      ...
      LVT = LWT
      IF (INFO(16) .NE. 0 .OR. 
     *     (INFO(18).EQ.1.AND.INFO(30).GT.0)) LVT = LWT + MYNEQ
      LPHI = LVT + MYNEQ
...
c === local variable
      integer i, myid, numprocs, myi, np
      do i = 1, isenfo(4)
         wrk(i) = 0.0d0
      end do
      myid = isenfo(8)
      ...
      do i = 1, isenfo(1)
         ...
         if (myi .le. np) wrk(myi) = 1.0d0
         call g_res(t,y,y(1+i*Ny),yp,yp(1+i*Ny),
     1        delta,delta(1+i*isenfo(6)),ires,rpar,ipar,senpar,wrk)
         if (myi .le. np) wrk(myi) = 0.0d0
      end do
      return
      end
c
...
C===================================================================
      ...
      real*8  wrk(*)            ! work array, 
                                ! size = isenfo(1)*(2*ny+isenfo(4)+nq)
c === local variable
      integer i, np, nq, ig_y, ig_yp,ig_delta,
