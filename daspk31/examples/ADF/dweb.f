...
C
C alpha and beta are the sensitivity parameters. In this example, we also 
...
C
      ...
      EXTERNAL RESWEB, JACRS, PSOLRS, H_RESW, I_RESW, K_RESW
C
...
C
      PARAMETER (MAXP=20,MAXS=2,MAXM=20, mxeq = MAXS*MAXM*MAXM,  
     *     MAXN = (MAXP+1)*mxeq,
     1     LRW = 364050,
     2     LIW = 5642)
C
      DIMENSION CC(MAXN), CCPRIME(MAXN), RWORK(LRW), IWORK(LIW),
     1     INFO(30), RPAR(mxeq), IPAR(4), rtol(1), atol(1),
     2     SENPAR(2)
C
...
C
C Initialize the sensitivity with the initial conditions
      DO I = 3, INFO(19)
         CC(I*NY+ 2*I + 1) = 1.0D0
      END DO
C
...
c           write(10, *) (cc(j), j=ny+1, 2*ny)
 ...
 200    CONTINUE
        if (info(19) .gt. 0) then
           ...
           sum2 = 0.0d0
           do i = 1, ny
              sum1 = sum1 + 2.0d0*cc(i)*cc(i+ny)
              sum2 = sum2 + 2.0d0*cc(i)*cc(i+2*ny)
           end do
           print *, sum1, sum2
        end if
C
...
C
      subroutine h_resw(np, t, y, g_y, yp, g_yp, 
     *     cj, delta, g_delta, ires,
     *     rpar, ipar, senpar, g_senpar)
      implicit none
      integer  np, ires, ipar(*)
      double precision t, y(*), yp(*), delta(*), cj, rpar(*), senpar(*),
     *     g_y(np,*), g_yp(np,*), g_delta(np,*),
     *     g_senpar(np,*)
c
c...  local variables 
      double precision g_rpar(802*20)

      
      call h_res(np, t, y, g_y, np, yp, g_yp, np,
     *     cj, delta, g_delta, np, ires,
     *     rpar, g_rpar, np, ipar, senpar, g_senpar, np)

      return
      end
c
      subroutine k_resw(t, y, g_y, yp, g_yp, 
     *     cj, delta, g_delta, ires,
     *     rpar, ipar, senpar, g_senpar)
      implicit none
      integer  ires, ipar(*)
      double precision t, y(*), yp(*), delta(*), cj, rpar(*), senpar(*),
     *     g_y(*), g_yp(*), g_delta(*),g_senpar(*)
c
c...  local variables 
      double precision g_rpar(800)

      
      call k_res(t, y, g_y, yp, g_yp, cj, delta, g_delta, ires,
     *     rpar, g_rpar, ipar, senpar, g_senpar)
