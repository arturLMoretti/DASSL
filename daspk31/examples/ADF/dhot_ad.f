...
C Load workspace lengths.
      PARAMETER (MAXM = 60, MAXM2 = MAXM+2, MXNEQ=MAXM2*MAXM2)
...
c...  required initial consistent values of ady and adyprime, set infob(2)=1:
      infob(2) = 0
      infob(7) = 180            ! estimate of the number of time steps
      infob(4) = 1
c
...
c
      print *, ' value of cost function at t=', tout
      print *, '      ', (qsen(i), i = 1, nq)
      lsen = nq*(neq+1)+1
      print *, ' sensitivities are:'
      print *, '      ', (qsen(lsen+k),k=0,nq-1)
      do k = 0, m+1
         do j = 1, m+2
            i = k*(m+2) + j
...
C
     ...
     *     qsen(*)
      integer ipar(*)
      integer ires, i, neq, M

      neq = ipar(33)
      M = IPAR(34)
      qsen(1) = 0.0d0
      do i = 1, M+1
         qsen(1) = qsen(1) + 0.5d0*(y(i)*y(i)+y(i+1)*y(i+1))/(M+1)
      end do
      return
      end
C
...
C
C     call TAMC generated routine
      call adpres(y, ipar, addelta, adsenpar)
