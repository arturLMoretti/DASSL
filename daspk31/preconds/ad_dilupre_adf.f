      SUBROUTINE AD_DJACILU (
     .     ADRES, IRES, NEQ, T, Y, YPRIME, REWT, SAVR,
...
C ... Calculate Jacobian matrix.
      IERR = 0
      CALL AD_DJCALC (
     .     NEQ, T, Y, YPRIME, SAVR, LBW, UBW, WK, REWT,ADRES,
     .     H, CJ, NNZMX, WP(LJAC), IWP(LJACJ), IWP(LJACI),
     .     WP(LPLU), IWP(LJLU), IWP(LJU), IPAR, RPAR,
     .     SENPAR, IRES, NRE, IPAR(LJADF), ADI, IERR, 
     .     ISENFO,SENWRK,ISENWK,G_RES, P_RES)
      IF (IRES .LT. 0) RETURN
      IF (IERR .NE. 0) RETURN
...
C------------  End of Subroutine DJACILU  ------------------------------
      ...
      SUBROUTINE AD_DJCALC (
     .     NEQ, T, Y, YPRIME, R0, ML, MU, R1, REWT, ADRES,
     .     H, CJ,  NNZMX, JAC, JA, IA, RCOO, JCOO, ICOO,
     .     IPAR, RPAR, SENPAR, IRES, NRE, IADF, ADI, IERR, 
     .     ISENFO,SENWRK,ISENWK,G_RES, P_RES)
...
C ... Local variables:
      ...
      if (ipar(12) .eq. 1) then
         ...
         ires = 0
         call jackbyAdf(
     1        NEQ, T, ADI, ADI(NY+1), R1, I_RES, CJ, JAC, JA, IA, 
     2        RCOO, JCOO, ICOO,
     3        IPAR, RPAR, SENPAR, IRES, IADF, IPAR(13), -1)
         if (isenfo(7) .eq. 1) then ! index-2 case
            do i = 1, neq
               do j = icoo(i), icoo(i+1)-1
                  if (isenwk(2+i).eq.-2) then
                     rcoo(j) = rcoo(j)*cj
                  else if (isenwk(2+i) .gt. 0) then
                     jj = jcoo(j)
...
C     R1(1),...,R1(NEQ) contains the perturbed values of RES.
      ...
      DO 40 J = 1,MBA
         DO 10 JJ = J,NEQ,MBAND
            ...
            DEL = SQUR*MAX(ABS(Y(JJ)),ABS(H*YPRIME(JJ)),
     *           ABS(1.0/REWT(JJ)))
