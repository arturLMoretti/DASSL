...
C------------  End of Subroutine GSET1  --------------------------------
      ...
      SUBROUTINE DRBGJA (
     *     T, U, R0, RBLOCK, R1, REWT, CJ, BD, IPBD, IER, SENPAR)
...
C      SUBROUTINE RBLOCK (T, JX, JY, UXY, RXY, SENPAR)
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      EXTERNAL RBLOCK
      DIMENSION U(*), R0(*), R1(*), REWT(*),BD(*),IPBD(*),SENPAR(*)
C
...
C In this case, change R0(J0+I) below to R0(I).
          DO 20 JS = 1,MP
            ...
            U(J) = U(J) + DEL
            FAC = -1.0D0/DEL
            CALL RBLOCK (T, JX, JY, U(J0+1), R1, SENPAR)
            DO 10 I = 1,MP
