...
C --------------------------------------------------------------------------
c
c...  interface to the TAPENADE-generated routines
c
C --------------------------------------------------------------------------
     ...
     *     ady(*), addelta(*), delta(*)
      double precision deltab, delta_copy, ady_sav(800)
      dimension deltab(800), delta_copy(800) 
      integer ires, ipar(*), i
...
C      Differentiation of resweb in reverse (adjoint) mode:
C       gradient, with respect to input variables: u delta
C       of linear combination of output variables: delta
      call RESWEB_B(t, y, ady, yp, cj, delta_copy, deltab, 
     +              ires, rpar, ipar, senpar)
...
C --------------------------------------------------------------------------
      SUBROUTINE RES_ADYP(
     *            T, Y, YPRIME, ADYP, CJ, DELTA, ADDELTA,
     *            IRES, RPAR, IPAR, SENPAR)
C                        SUBROUTINE RES_ADYPRIME(
C                      *            T, Y, YPRIME, ADYPRIME, CJ, DELTA, ADDELTA,
C                      *            IRES, RPAR, IPAR, SENPAR)
      implicit none
      double precision t, y(*), yprime(*), cj, rpar(*), 
     *     senpar(*), adyp(*), addelta(*), delta(*)
      double precision DELTAB, DELTA_COPY, adyp_sav(800)
      dimension DELTAB(800), DELTA_COPY(800)
      double precision adyp_tap(800)
      integer ires, ipar(*), I
...
C     Differentiation of resweb_ofyp in reverse (adjoint) mode:
C      gradient, with respect to input variables: delta uprime
C      of linear combination of output variables: delta
C
      call RESWEB_OFYP_B(t, y, yprime, adyp, cj, delta_copy, deltab
     +                   , ires, rpar, ipar, senpar)
     
      do i = 1, 800
         adyp(i) = adyp(i) + adyp_sav(i)
      end do
...
C --------------------------------------------------------------------------
      ...
      SUBROUTINE RES_ADP(
     *             T, Y, YP, CJ, DELTA, ADDELTA, IRES, RPAR, 
     *             IPAR, SENPAR, ADP)
      implicit none
      double precision t, y(*), yp(*), cj, rpar(*), senpar(*),
     *     adp(*), addelta(*), delta(*)
      double precision DELTAB, DELTA_COPY
      dimension DELTAB(800), DELTA_COPY(800)
      double precision adp_sav(800)
      integer ires, ipar(*), I
...
C  Differentiation of resweb_ofp in reverse (adjoint) mode:
C   gradient, with respect to input variables: senpara delta
C   of linear combination of output variables: delta
      CALL RESWEB_OFP_B(t, y, yp, cj, delta_copy, deltab,
     +               ires, rpar, ipar, senpar, adp)
     
      do i = 1 , 800
        adp(i) = adp(i) + adp_sav(i)
      end do
