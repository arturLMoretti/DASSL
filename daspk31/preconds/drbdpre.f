...
C      SUBROUTINE RBLOCK (T, JX, JY, UXY, RXY, senpara)
...
C------------  End of Subroutine DMSET2  -------------------------------
      ...
      SUBROUTINE DRBDJA (
     *     T, U, R0, RBLOCK, R1, REWT, CJ, BD, IPBD, IER, senpara)
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      EXTERNAL RBLOCK
      DIMENSION U(*), R0(*), R1(*), REWT(*), BD(*),IPBD(*),senpara(*)
     *     
C
      COMMON /DRPRE1/ SRUR, MP, MPD, MPSQ, MESHX, MESHY, MXMP
C
...
C In this case, change R0(J0+I) below to R0(I).
          DO 20 JS = 1,MP
            ...
            U(J) = U(J) + DEL
            FAC = -1.0D0/DEL
            CALL RBLOCK (T, JX, JY, U(J0+1), R1, senpara)
            DO 10 I = 1,MP
