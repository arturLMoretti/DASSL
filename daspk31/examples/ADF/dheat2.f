...
C take some of the initial conditions as sensitivity parameters. 
C We have two derived functions in this example
C    g(1) = sum_1^N (u_i*u_i)
C    g(2) = \int sum(u_i)
C
C g(2) is taken as a quadrature variable.
C-----------------------------------------------------------------------
C
C***ROUTINES CALLED
C   UINIT, DDASPK
...
C
      ...
      EXTERNAL RESH, DBANJA, DBANPS, sg_RES_proc, 
     *     gh_RES_proc, G_RES_proc, QRES, gq_QRES_proc
c
c     maximum number of the sensitivity parameters
      parameter (Npara=20)
      PARAMETER (MAXM = 50, MAXM2 = MAXM+2, MXNEQ=(npara+6)*MAXM2*MAXM2)
      PARAMETER (LENRW = 91 + 18*MXNEQ, LENIW  = 10630,
     *          LENWP  = 5*MXNEQ + 2*((MXNEQ/3) + 1), LENIWP = MXNEQ)
      DIMENSION U(MXNEQ),UPRIME(MXNEQ), iwork(leniw), 
     *     RWORK(LENRW+LENWP)   ! IWORK(LENIW+LENIWP)
      DIMENSION INFO(30), RPAR(10), IPAR(10), SENPAR(2), QSEN(100)
...
c---------------------------------------------------------------
      ...
      print *, ' method for computing sensitivity residues:(0,1,2,3,4) '
      ...
      print *, '    4: Adifor with matrix-vector product option'
      read (*,*) info(inso+2)
      ...
      read(*, *) info(inso+5)
      info(inso+6) = 1          ! no derived information
      info(inso+7) = 1          ! simultaneous corrector(0), staggered(1)
      info(inso+10) = 1         ! number of the quadrature variables
 25   continue
      if (info(28).gt.0.and.info(20).eq.2) info(20) = 0
      ny = (ny+info(28))
      neq = ny*(info(19) + 1) 
c
c...  intialization for index-0 case
      info(11) = 1
      do i = 1, neq
         iwork(40+i) = 1
      end do
...
C
C  Starting from the third one, the sensitivity parameters are
C  all in the initial conditions. Therefore, the initial values for
C  the sensitivity variables can be 1.0, for points 13--27 in row 21.
C
      do i = 4, info(19)+1
         u((i-1)*ny + 20*(M+2) + i + 9) = 1.0d0
      end do
C
...
C
      ...
      DO 70 IOUT = 1,NOUT
         IF (INFO(12) .EQ. 0) then
            if (info(20) .eq. 3) then
               CALL DDASPK (
     1              RESH, NEQ, T, U, UPRIME, TOUT, INFO, RTOL, ATOL,
     1              IDID, RWORK, LRW, IWORK, LIW, RPAR,IPAR,sg_RES_proc, 
     1              DBANPS,SENPAR, gh_RES_proc)
            ELSE 
               CALL DDASPK (
     1              RESH, NEQ, T, U, UPRIME, TOUT, INFO, RTOL, ATOL,
     1              IDID, RWORK, LRW, IWORK, LIW, RPAR, IPAR,
     I              sg_RES_proc,
     1              DBANPS,SENPAR, G_RES_proc)
            end if
     ...
     1              DBANPS,SENPAR, G_RES_proc)
         ...
         if (info(20) .eq. 3) then
            CALL DSENSD(
     *           gq_QRES_proc, NEQ, T, U, UPRIME, QSEN, INFO, 
     *           RWORK, IWORK, IRES, RPAR, IPAR, SENPAR)
         else if (info(20). lt. 2) then
            CALL DSENSD(
     *           QRES, NEQ, T, U, UPRIME, QSEN, INFO, 
     *           RWORK, IWORK, IRES, RPAR, IPAR, SENPAR)
         end if
C
         if (info(28).gt.0) then
            print *, '      sensitivity of integral (sum (u_i) :'
            print *, '              ', (u(ny*i), i=2, 3)
         end if
         if (info(24) .gt. 0 .and. info(20).lt.4) then
            print *, '      sensitivity of sum_{u_i^2} :'
            print *, '              ', (qsen(i), i = 2, 3)
         end if
         UMAX = 0.0D0
         DO 50 I = 1,NEQ/(info(inso+1) + 1)-1
...
C
      NEQ = IPAR(3)
C     
C     np is the total number of sensitivity parameters
      NP  = IPAR(5)
      M = IPAR(4)
      M2 = M + 2
      DX = RPAR(1)
      COEFF = RPAR(2)
C
C  The first two parameters appear in the RES routine
      do i = 1, (np+1)*neq
         u(i) = 0.0d0
         uprime(i) = 0.0d0
      end do
C
C  initial values for the internal pointers of the domain
      DO 20 K = 1,M
        YK = K*DX
        IOFF = M2*K
        DO 10 J = 1,M
...
C------------  End of Subroutine UINIT  --------------------------------
     ...
     *                  SENPAR)
C
C This routine computes the residuals for the 2-D discretized heat equation,
...
C
C This routine computes the residuals for the 2-D discretized heat equation,
C with zero boundary values. It also computes the residuals for the
