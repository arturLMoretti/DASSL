...
C-----------------------------------------------------------------------
     ...
     *                  WK, H, CJ, WP, IWP, IER, RPAR, IPAR, SENPARA)
C
...
C***REVISION DATE  991104   Added SENPARA(*)
...
C
      ...
      DIMENSION WP(*), IWP(*), RPAR(*), IPAR(*), SENPARA(*)
C
...
C
 ...
 10       CONTINUE
        CALL RES (T, Y, YPRIME, CJ, WK, IRES, RPAR, IPAR, SENPARA)
        IF (IRES .LT. 0) RETURN
        DO 30 N = J,NEQ,MBAND
          K = (N-J)/MBAND + 1
          Y(N) = WP(ISAVE+K)
...
C------------  End of Subroutine DBANJA  -------------------------------
     ...
     *                   WP, IWP, B, EPLIN, IER, RPAR, IPAR, SENPARA)
C
...
C
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      DIMENSION B(*),WP(*),IWP(*),RPAR(*),IPAR(*), SENPARA(*)
C
      ML = IPAR(1)
      MU = IPAR(2)
      MEBAND = 2*ML + MU + 1
