...
c     http://www.cwi.nl/cwi/projects/IVPtestset/src/problems/wheel.f
c
c     This is revision
c     $Id: wheel.f,v 1.1 1998/11/25 14:19:31 walter Exp $
c
c-----------------------------------------------------------------------
C=======================================================================
C
...
C      PURPOSE :        DRIVER ROUTINE FOR SIMULATION
C                       OF WHEELSET EXAMPLE WITH DASSL
...
C      INTEGRATOR:      DASSL
C      SUBROUTINES:     RESWHS (CALLS CREEP,PS1,PRAD1,VERTMA)
C
C=======================================================================
...
C=======================================================================
C     `CWI Test Set for IVP Solvers' IDE wrappers for DASSL
C=======================================================================
c
...
c-----------------------------------------------------------------------
      ...
      delta(12) = cj*delta(12)
      delta(13) = cj*delta(13)
      return
      end
c-----------------------------------------------------------------------
      subroutine feval(neqn,t,y,yprime,f,ierr,rpar,ipar,senpar)
      integer neqn,ierr,ipar(*)
      double precision t,y(neqn),yprime(neqn),f(neqn),rpar(*),senpar(*)
      double precision ytmp(17)
c
c     uses the routines
c
c          reswhs
...
c
      ipar(1)=0
      call reswhs(t,ytmp,yprime,f,ires,rpar,ipar,senpar)
      return
      end
c-----------------------------------------------------------------------
...
c
      ...
      SUBROUTINE RESWHS ( T, Y, DY, DELTA, IRES, RPAR, IPAR, SENPAR )
C====================================================================
C
C     RESWHS
C     ======
...
C
      DOUBLE PRECISION
     *           T, Y(1:17), DY(1:17), DELTA(1:17), RPAR, SENPAR(*)
      INTEGER    IRES, IPAR(*)
      DOUBLE PRECISION
...
C     *           XL    = 0.19d0,
C
C...  Initialize MU and XL by the SENPAR(*) array
      MU      = SENPAR(1)
      XL      = SENPAR(2)
C
C =====================================================================
...
C
      DELTA(6) = MR*(-XXPP + V*V*S*COA*(1.0d0 + (XX*COA-YY*SIA)*S)
     *            + 2.0D0*V*S*COA*ZZP)
     *            + TL(1) + TR(1) + FQ1 - MR*G*SIA
     *            + QL(1)*E1 + QR(1)*E2 - 2.0d0*CX*XX
      DELTA(7) = MR*(-YYPP -V*V*S*SIA*(1.0d0 + (XX*COA-YY*SIA)*S)
     *            -  2.0D0*V*S*SIA*ZZP)
     *            + TL(2) + TR(2) + FQ2 - MR*G*COA
     *            + QL(2)*E1 + QR(2)*E2
      DELTA(8)  = MR*(-ZZPP - 2.0d0*V*S*(XXP*COA-YYP*SIA)
     *                + V*V*S*S*ZZ)
     *            + TL(3) + TR(3) + FA + FQ3
     *            + QL(3)*E1 + QR(3)*E2
C
...
C
     ...
     *            + (XRR*SIT*SIP-RXR*COSR*SIT*COP)*TR(3)
      DELTA(10) = - LI2*(PHIPP - TETAP*V*S*SIA*COT)
