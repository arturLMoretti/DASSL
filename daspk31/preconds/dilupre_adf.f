...
C      IPAR(12) = MTYPE     - Flag for evaluating the Jacobian matrix:
C                             0 --- finite differencing
C                             1 --- ADIFOR evaluation, and the IPAR(13)
C                                   must be set as :
C                                   if RPAR depends on Y or YPRIME, 
C                                      set IPAR(13) = size of RPAR(*) 
C                                   else set IPAR(13) = 0.
C                             If ADIFOR option is chosen, you must 
C                             dimension IPAR(*) properly (see (e)).
C
C      IPAR(13) = NRPD      - If RPAR depends on Y or YPRIME, 
C                                set IPAR(13) = size of RPAR(*) 
C                             else set IPAR(13) = 0.
C      IPAR(14) = NSAVE     - If the Jacobian should be saved for
C                             later use, set IPAR(14) = 1; 
C                             Otherwise, set IPAR(14) = 0.
C      IPAR(15) = LADF      - Start position in IPAR array for ADIFOR 
C                             with SparseLinC option. Used only when
C                             IPAR(12) = 1
C      IPAR(16) = NIPD      - Dimension for IPAR. Used only when ADIFOR is
C                             chosen to evaluate the Jacobian.
...
C
C  (e) If ADIFOR with sparseLinc is used to evaluate the Jacobian, 
C      dimension the IPAR to proper size, added 3*NEQ + IPAR(13) to 
C      the original value. The ADIFOR work space will start from IPAR(15).
C 
...
c ... Load values from IPAR and RPAR.  Check for illegal values.
      ...
      IF ((JSCALCOL .LT. 0) .OR. (JSCALCOL .GT. 1)) THEN
         ...
         RETURN
      ENDIF
      IF (IPAR(12) .LT. 0 .OR. IPAR(12) .GT. 1) THEN ! MTYPE
         IERR = 12
      END IF
      if (ipar(13) .lt. 0) then
         ierr = 13
         return
      end if
      if (ipar(14) .lt. 0 .and. ipar(14) .gt. 1) then
         ierr = 14
         return
      end if
      if (ipar(15) .lt. 30) then ! start of the ADIFOR array
         ierr = 15
         return
      end if
      IF (IPAR(12) .EQ. 1) THEN
         IF (IPAR(16) .LT. IPAR(15) + 3*NEQ + IPAR(13)) THEN
            IERR = 16
            RETURN
         END IF
         DO I = IPAR(15), IPAR(15) + 3*NEQ + IPAR(13)
            IPAR(I) = 0
         END DO
      END IF
c
...
c ... Set up pointers into WP
      LJAC = 1
      LROWNRMS = NNZMX + LJAC
      IF (IPAR(14) .EQ. 1) LROWNRMS = LROWNRMS + NNZMX
C      LROWNRMS = 2*NNZMX + LJAC
...
c ... Set up pointers into IWP
      LJACI = 1
      LJACJ = LJACI + NEQP1
      IF (IPAR(14) .EQ. 1) LJACJ = LJACJ + NEQP1
      LJU = LJACJ + NNZMX
      IF (IPAR(14) .EQ. 1) LJU = LJU + NNZMX
...
c      IF (IPAR(12) .EQ. 1) LIWP_MIN = LIWP_MIN + 3*NEQ + IPAR(13)
...
C------------  End of Subroutine DSPSETUP  -----------------------------
      ...
      SUBROUTINE DJACILU (
     .     RES, IRES, NEQ, T, Y, YPRIME, REWT, SAVR,
     .     WK, H, CJ, WP, IWP, IERR, RPAR, IPAR, SENPAR)
...
C ... Local variables:
     ...
     *        LROWNRMS, LRWK1, LIWK1, IFMT,NSAVE
      INTEGER LENPFAC, LENPLUFAC, LFILILUT, IPREMETH, NEQP1, NNZMX
      INTEGER LPLU, LJU, LJLU, LPERM, LQPERM, LLEVELS, LMASK, LJADF
      INTEGER ISRNORM  !=1 causes row normalization of Jac.
      INTEGER NORMTYPE !=0,1,2 for max-norm, 1-norm, or
                       ! 2-norm row scaling
      INTEGER IREORDER !=1 causes row and column reordering of Jac.
      ...
      CHARACTER*8 PMETH(2), PREMETH
      CHARACTER*72 TITLE
      CHARACTER*80 MSG
      SAVE
      DATA PMETH/'ILUT','ILUTP'/
...
C ... Set up pointers into IWP
      LJACI = 1
      IF (NSAVE .EQ. 1) LJACI = NEQP1+LJACI
      LJACJ = LJACI + NEQP1
      IF (NSAVE .EQ. 1) LJACJ = LJACJ + NNZMX
C      LJADF = LJACJ + NNZMX
c      IF (IPAR(12) .EQ. 0) THEN
c         LJU = LJADF
c      ELSE
c         LJU = LJADF + 3*NEQ + IPAR(13)
c      END IF
      LJADF = IPAR(15)
      LJU = LJACJ + NNZMX
      LJLU = LJU + LENPLUMX
C ... Calculate Jacobian matrix.
     ...
     .             SENPAR, IRES, NRE, IPAR(LJADF), IERR)
      IF (IRES .LT. 0) RETURN
      IF (IERR .NE. 0) RETURN
C ... save the Jacobian for used in Krylov method
C
      IF (NSAVE .EQ. 1) 
     *     CALL ATOB(NEQ, WP(LJAC), IWP(LJACJ), IWP(LJACI),
     *     WP, IWP(2*NEQP1+1), IWP)
...
C------------  End of Subroutine DJACILU  ------------------------------
      ...
      SUBROUTINE DJCALC (
     .     NEQ, T, Y, YPRIME, R0, ML, MU, R1, REWT, RES,
     .     H, CJ,  NNZMX, JAC, JA, IA, RCOO, JCOO, ICOO,
     .     IPAR, RPAR, SENPAR, IRES, NRE, IADF, IERR)
...
C ... Workspace for temporary storage of Jacobian elements:
      ...
      INTEGER IADF(*)      ! work space for ADIFOR
C ... Local variables:
      ...
      if (ipar(12) .eq. 1) then
         call jackbyAdf(
     1        NEQ, T, Y, YPRIME, R1, I_RES, CJ, JAC, JA, IA, 
     2        RCOO, JCOO, ICOO,
     3        IPAR, RPAR, SENPAR, IRES, IADF, IPAR(13), 0)
         return
      end if
...
C     R1(1),...,R1(NEQ) contains the perturbed values of RES.
 ...
 10       CONTINUE
        CALL RES (T, Y, YPRIME, CJ, R1, IRES, RPAR, IPAR, SENPAR)
        IF (IRES .LT. 0) RETURN
        NRE = NRE + 1
...
C------------  End of Subroutine DJCALC  -------------------------------
     ...
     .     WP, IWP, BL, EPLIN, IERR, RPAR, IPAR, SENPAR)
...
C------------  End of Subroutine DJREORD  ------------------------------
      END
c
c...  Simple scaling preconditioner
c
      subroutine DPSOL (NEQ, T, Y, YPRIME, R0, WK, CJ, WGHT, 
     .     WP, IWP, BL, EPLIN, IERR, RPAR, IPAR, SENPAR)
c ... Version of 10-6-95
...
C ... Local variables:
      ...
      SUBROUTINE jackbyAdf (
     1     NEQ, T, Y, YPRIME, R1, I_RES, CJ, JAC, JA, IA, 
     2     RCOO, JCOO, ICOO, IPAR, RPAR, SENPAR, IRES, IWK, NRPD, iad)

c ... Version of 02-20-00
...
C ... Input arguments:
      INTEGER NEQ               ! number of equations for state variables
      REAL*8 T                  ! independent variable t
      REAL*8 Y(NEQ)             ! most recent iterate of solution vector y
      REAL*8 YPRIME(NEQ)        ! most recent iterate of solution vector y'
...
C ... Output arguments:
      ...
      INTEGER IAD
C ... Workspace for temporary storage of Jacobian elements:
      REAL*8 RCOO(*)   ! nonzero Jacobian elements
      INTEGER JCOO(*)  ! col indices of nonzero Jacobian elements
      INTEGER ICOO(*)  ! row indices of nonzero Jacobian elements
c ... Workspace for  adifor without sparslink
*      pointer (ipiwk, iwk)
      integer iwk(*)            ! integer work array, size>=3*neq+nrpd
c === local variable
      integer i, j, ig_y, ig_yp, ig_delta,  ig_rpar, jj, itotal
      integer ny, nnz, idebug
      real*8  sum
      data idebug/0/   
c
      ny = neq
...
c
      if (nrpd .eq. 0) then
         call i_res(
     $        t, y, iwk(ig_y+1), yprime,  iwk(ig_yp+1), cj, r1, 
     $        iwk(ig_delta+1), ires, rpar, ipar, SENPAR)
      else 
         call i_res(
     $        t, y, iwk(ig_y+1), yprime, iwk(ig_yp+1), cj, r1, 
     $        iwk(ig_delta+1), ires, 
     $        rpar,iwk(ig_rpar+1),ipar, SENPAR)
      end if
      if (ires .lt. 0) return
c
...
c      call ehrpt
      ...
      if (iad .lt. 0) then
c
c... adjoint integration, require transpose
         do i = 1, neq
            call dspxsq(icoo(nnz+1),rcoo(nnz+1),ny,
     *           iwk(ig_delta+i),jj,ires)
            if (ires .lt. 0) then
               write(*,*) ' error in jackbyAdf_da() '
               stop
            end if
            if (idebug .eq. 1) then
               sum = 0.0d0
               do j = 1, jj
                  sum = sum + rcoo(nnz+j)*rcoo(nnz+j)
               end do
               if (sum .eq. 0.0d0) then
                  write(*,*) ' In jacbyAdf: Jacobian has a zero-row! '
                  write(*,*) '              row number = ', i
                  ires = -2
                  return
               end if
...
C ... Convert Jacobian from coordinate to compressed sparse row format.
         ...
         ia(1) = 1
         do i = 1, neq
            call dspxsq(ja(nnz+1),jac(nnz+1),ny,iwk(ig_delta+i),jj,ires)
            if (ires .lt. 0) then
               write(*,*) ' error in jackbyAdf_da() '
               stop
            end if
